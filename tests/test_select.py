...
async def test_select_model_with_whereclause(
    ...
    item = populated_db[0]
    result = await crud_ins.select_model(async_db_session, item.id, crud_ins.model.del_flag == False)
...
async def test_select_model_by_column_with_whereclause(
    ...
    item = populated_db[0]
    result = await crud_ins.select_model_by_column(async_db_session, crud_ins.model.del_flag == False, name=item.name)
...
async def test_select_models_with_whereclause(
    ...
    results = await crud_ins.select_models(async_db_session, crud_ins.model.del_flag == False)
...
async def test_select_models_order_with_whereclause(
    ...
    results = await crud_ins.select_models_order(async_db_session, 'name', crud_ins.model.del_flag == False)
...
async def test_count_with_whereclause(async_db_session: AsyncSession, populated_db: list[Ins], crud_ins: CRUDPlus[Ins]):
    count = await crud_ins.count(async_db_session, crud_ins.model.del_flag == False)
...
async def test_exists_with_whereclause(
    ...
    exists = await crud_ins.exists(async_db_session, crud_ins.model.del_flag == False)
